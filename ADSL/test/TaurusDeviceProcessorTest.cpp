...
TEST_F(TaurusDeviceProcessorTest, test_emptyMessage) {
    ASSERT_TRUE(m_processor->decodeVolatileIdentificationCode("").empty());
}
...
TEST_F(TaurusDeviceProcessorTest, test_decodeNonEmptyIdentificationCode) {
    ...
    ASSERT_EQ("1234567890", m_processor->decodeVolatileIdentificationCode(message));
}
